...
import { Pencil, Trash2, ChevronLeft, ChevronRight, X, Wallet, User, Users, Settings2 } from "lucide-react";
...
export default function FinanceApp() {
    ...
    (async () => {
      try {
        const [tx, acc, cat] = await Promise.allSettled([
          ...
          window.storage.get("accounts", false),
  ...
  async function persistTransactions(next) {
    ...
    try {
      await window.storage.set("transactions", JSON.stringify(next), false);
    } catch (e) {
  ...
  async function persistAccounts(next) {
    setAccounts(next);
    try {
      await window.storage.set("accounts", JSON.stringify(next), false);
    } catch (e) {
      setError("Gagal menyimpan daftar akun.");
    }
  }
  ...
  async function persistCategories(next) {
    setCategories(next);
    try {
      await window.storage.set("categories", JSON.stringify(next), false);
    } catch (e) {
      setError("Gagal menyimpan daftar kategori.");
    }
  }
  ...
  function handleTypeChange(type) {
    setForm((f) => ({
      ...f,
      type,
      category: categories[activeScope][type][0] || "",
    }));
  }
  ...
  async function addCategory() {
    ...
    const next = {
      ...categories,
      [activeScope]: { ...categories[activeScope], [form.type]: [...list, name] },
    };
  ...
  async function removeCategory(name) {
    ...
    const next = {
      ...categories,
      [activeScope]: { ...categories[activeScope], [form.type]: nextList },
    };
    await persistCategories(next);
    if (form.category === name) {
      setForm((f) => ({ ...f, category: nextList[0] }));
    }
  }
  ...
  async function removeAccount(name) {
    ...
    await persistAccounts(next);
    if (form.account === name) {
      setForm((f) => ({ ...f, account: nextList[0] }));
    }
  }
  ...
  // ---------- derived data (scoped to activeScope) ----------
  ...
  const scopeSaldo = useMemo(
    () => scopeTx.reduce((s, t) => s + (t.type === "pemasukan" ? t.amount : -t.amount), 0),
    [scopeTx]
  );
  ...
  const accountBalances = useMemo(() => {
    ...
    scopeTx.forEach((t) => {
      map[t.account] = (map[t.account] || 0) + (t.type === "pemasukan" ? t.amount : -t.amount);
    });
  ...
  const categoryChartData = useMemo(() => {
    ...
    monthTx
      .filter((t) => t.type === chartTab)
      .forEach((t) => {
        map[t.category] = (map[t.category] || 0) + t.amount;
      });
    return cats
      .map((c) => ({ name: c, total: map[c] }))
      .filter((c) => c.total > 0)
      .sort((a, b) => b.total - a.total);
  }, [monthTx, chartTab, scopeCategories]);

  const sortedMonthTx = useMemo(
    () => [...monthTx].sort((a, b) => (a.date < b.date ? 1 : a.date > b.date ? -1 : 0)),
    [monthTx]
  );
  ...
  if (loading) {
    return (
      <div style={{ ...styles.page, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ color: "#8A7B60", fontSize: 14 }}>Memuat catatan keuangan...</div>
      </div>
    );
  }
  ...
  return (
    <div style={styles.page}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        input:focus, select:focus, textarea:focus, button:focus-visible {
          outline: 2px solid #B98D3E; outline-offset: 1px;
        }
        ::placeholder { color: #9C8F7A; }
        .fin-scrollbar::-webkit-scrollbar { width: 8px; height: 8px; }
        .fin-scrollbar::-webkit-scrollbar-thumb { background: #D8CDB4; border-radius: 4px; }
      ...
      <header style={styles.header}>
        <div style={styles.headerInner}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              <div style={styles.headerTitle}>Buku Kas Digital</div>
              <div style={styles.headerSubtitle}>Catatan pemasukan &amp; pengeluaran harian</div>
            </div>
          </div>
          <div style={{ ...styles.stamp, borderColor: `${scopeAccent}A6` }}>
            <div style={{ ...styles.stampInner, color: scopeAccent === "#B98D3E" ? "#E9C77E" : "#AFCBE8" }}>
        ...
        {/* Scope tabs */}
        <div style={styles.scopeTabsWrap}>
          <div style={styles.scopeTabs}>
            {Object.keys(SCOPE_META).map((key) => (
              <button
                ...
                style={{
                  ...styles.scopeTabBtn,
                  ...(activeScope === key ? styles.scopeTabActive : {}),
                }}
              >
      ...
      <main style={styles.main} className="fin-grid">
        {/* Left: Slip transaksi */}
        <section style={styles.slip}>
          <div style={styles.slipPerforation} />
          <div style={{ padding: "22px 22px 24px" }}>
            <div style={styles.slipHeaderRow}>
              <h2 style={styles.slipTitle}>{form.id ? "Ubah Transaksi" : "Slip Transaksi"}</h2>
              {form.id && (
                <button type="button" onClick={resetForm} style={styles.iconBtnGhost} aria-label="Batalkan ubah">
                  <X size={16} />
                </button>
              )}
            </div>
            <div style={styles.slipScopeNote}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={styles.toggleRow}>
                <button
                  ...
                  style={{
                    ...styles.toggleBtn,
                    ...(form.type === "pemasukan" ? styles.toggleBtnActiveIncome : {}),
                  }}
                >
                  + Pemasukan
                </button>
                <button
                  ...
                  style={{
                    ...styles.toggleBtn,
                    ...(form.type === "pengeluaran" ? styles.toggleBtnActiveExpense : {}),
                  }}
                >
                  − Pengeluaran
                </button>
              ...
              <div style={styles.labelRow}>
                <label style={{ ...styles.label, margin: 0 }}>Rincian (kategori)</label>
                <button
                  type="button"
                  onClick={() => setManageOpen((m) => ({ ...m, category: !m.category, account: false }))}
              ...
              {manageOpen.category && (
                <div style={styles.managePanel}>
                  <div style={styles.manageHint}>
                    Kategori {form.type} — {SCOPE_META[activeScope].shortLabel}
                  </div>
                  ...
                  <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                    <input
                      ...
                      style={{ ...styles.input, flex: 1, background: "#fff" }}
                    />
                    <button type="button" onClick={addCategory} style={styles.smallBtn}>
                      Tambah
                    </button>
                  </div>
                </div>
              ...
              <textarea
                placeholder={
                  activeScope === "pribadi"
                    ? "Contoh: Makan siang bersama tim di kantor"
                    : "Contoh: Sewa kamera + lensa untuk syuting hari 1"
                }
                value={form.detail}
              ...
              <div style={{ position: "relative" }}>
                <span style={styles.rpPrefix}>Rp</span>
                <input
                  type="text"
                  inputMode="numeric"
                  placeholder="0"
                  ...
                  style={{ ...styles.input, paddingLeft: 40, fontFamily: "'IBM Plex Mono', monospace" }}
                />
              </div>

              {/* Akun + kelola */}
              <div style={styles.labelRow}>
                <label style={{ ...styles.label, margin: 0 }}>Sumber / tujuan dana</label>
                <button
                  type="button"
                  onClick={() => setManageOpen((m) => ({ ...m, account: !m.account, category: false }))}
                  style={styles.manageLink}
                >
              ...
              {manageOpen.account && (
                <div style={styles.managePanel}>
                  <div style={styles.manageHint}>Akun — {SCOPE_META[activeScope].shortLabel}</div>
                  <div style={styles.chipWrap}>
                    {scopeAccounts.map((a) => (
                  ...
                  <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                    <input
                      ...
                      style={{ ...styles.input, flex: 1, background: "#fff" }}
                    />
                    <button type="button" onClick={addAccount} style={styles.smallBtn}>
                      Tambah
                    </button>
                  </div>
                </div>
        ...
        <section>
          ...
          <div style={styles.monthNav}>
            <button onClick={() => shiftMonth(-1)} style={styles.navBtn} aria-label="Bulan sebelumnya">
              <ChevronLeft size={18} />
            </button>
            <div style={styles.monthLabel}>
              {MONTHS_ID[cursor.month]} {cursor.year}
            </div>
            <button onClick={() => shiftMonth(1)} style={styles.navBtn} aria-label="Bulan berikutnya">
          ...
          <div style={styles.summaryRow}>
            <div style={{ ...styles.summaryCard, borderTop: "3px solid #3F7A5C" }}>
              <div style={styles.summaryLabel}>Pemasukan</div>
              <div style={{ ...styles.summaryValue, color: "#2C5A42" }}>{rupiah(monthIncome)}</div>
            </div>
            <div style={{ ...styles.summaryCard, borderTop: "3px solid #B54B3E" }}>
              <div style={styles.summaryLabel}>Pengeluaran</div>
              <div style={{ ...styles.summaryValue, color: "#9A3B2F" }}>{rupiah(monthExpense)}</div>
            </div>
            <div style={{ ...styles.summaryCard, borderTop: `3px solid ${scopeAccent}` }}>
              <div style={styles.summaryLabel}>Saldo Bulan Ini</div>
              <div style={{ ...styles.summaryValue, color: monthSaldo >= 0 ? "#2C5A42" : "#9A3B2F" }}>
                {rupiah(monthSaldo)}
              </div>
            </div>
          </div>
          ...
          {/* Account balances */}
          <div style={styles.accountRow} className="fin-scrollbar">
            {accountBalances.map((a) => (
              <div key={a.account} style={styles.accountChip}>
          ...
          <div style={styles.panel}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              <h3 style={styles.panelTitle}>Rincian per Kategori</h3>
              <div style={styles.chartTabs}>
                <button
                  onClick={() => setChartTab("pengeluaran")}
                  style={{ ...styles.chartTabBtn, ...(chartTab === "pengeluaran" ? styles.chartTabActive : {}) }}
                >
                  Pengeluaran
                </button>
                <button
                  onClick={() => setChartTab("pemasukan")}
                  style={{ ...styles.chartTabBtn, ...(chartTab === "pemasukan" ? styles.chartTabActive : {}) }}
                >
                  Pemasukan
                </button>
              </div>
            ...
            {categoryChartData.length === 0 ? (
              <div style={styles.emptyState}>Belum ada data {chartTab} pada bulan ini.</div>
            ) : (
              <div style={{ width: "100%", height: Math.max(120, categoryChartData.length * 40) }}>
                <ResponsiveContainer>
                  <BarChart data={categoryChartData} layout="vertical" margin={{ left: 8, right: 16, top: 4, bottom: 4 }}>
                    ...
                    <XAxis
                      type="number"
                      tickFormatter={(v) => (v >= 1000000 ? `${(v / 1000000).toFixed(1)}jt` : `${Math.round(v / 1000)}rb`)}
                      tick={{ fontSize: 11, fill: "#6B5F4C" }}
          ...
          {/* Transaction list */}
          <div style={styles.panel}>
            <h3 style={styles.panelTitle}>Transaksi {MONTHS_ID[cursor.month]}</h3>
            {sortedMonthTx.length === 0 ? (
              <div style={styles.emptyState}>
                Belum ada transaksi {SCOPE_META[activeScope].shortLabel.toLowerCase()} di bulan ini. Isi slip di sebelah kiri untuk mulai mencatat.
              </div>
            ) : (
              <div>
                {sortedMonthTx.map((t) => (
                  <div key={t.id} style={styles.txRow}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {t.detail && (
                        <div style={{ fontSize: 12.5, color: "#7A6E5A", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                          {t.detail}
                        </div>
                      )}
                    </div>
                    <div style={styles.txAccount}>{t.account}</div>
                    <div
                      style={{
                        ...styles.txAmount,
                        color: t.type === "pemasukan" ? "#2C5A42" : "#9A3B2F",
                      }}
                    >
                      {t.type === "pemasukan" ? "+" : "−"} {rupiah(t.amount)}
                    </div>
                    <div style={{ display: "flex", gap: 4 }}>
                      <button onClick={() => handleEdit(t)} style={styles.iconBtnGhost} aria-label="Ubah">
                        <Pencil size={14} />
                      </button>
                      <button onClick={() => handleDelete(t.id)} style={styles.iconBtnGhost} aria-label="Hapus">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </div>
          ...
          <div style={styles.footer}>
            {scopeTx.length > 0 && (
              <button onClick={handleResetScope} style={styles.resetLink}>
...
const styles = {
  ...
  header: {
    background: "linear-gradient(180deg, #1B3830 0%, #16302B 100%)",
    padding: "18px 20px 0",
  },
  headerInner: {
    maxWidth: 1080,
  ...
  stamp: {
    border: "2px dashed rgba(185,141,62,0.65)",
    borderRadius: "50%",
    width: 104,
    height: 104,
    display: "flex",
  ...
  scopeTabsWrap: {
    maxWidth: 1080,
    margin: "0 auto",
    borderTop: "1px solid rgba(246,241,228,0.12)",
  },
  scopeTabs: {
    display: "flex",
    gap: 4,
  ...
  slip: {
    ...
    boxShadow: "0 1px 2px rgba(43,35,32,0.04)",
    position: "sticky",
    top: 20,
  },
  slipPerforation: {
    height: 6,
    background:
      "repeating-linear-gradient(90deg, transparent, transparent 6px, #E4DAC4 6px, #E4DAC4 8px)",
